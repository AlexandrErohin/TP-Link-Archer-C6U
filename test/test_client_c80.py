...
class TestTPLinkClient(TestCase):
    ...
    def test_supports(self) -> None:
        ...
        class TplinkC80RouterTest(TplinkC80Router):
            def request(self, code: int, asyn: int, token: str = None, data: str = None) -> dict | None:
                # Response for supports
                if code == 2 and asyn == 1:
                    return ResponseMock(response_status)
        ...
        class TplinkC80RouterTest(TplinkC80Router):
            def request(self, code: int, asyn: int, token: str = None, data: str = None) -> dict | None:
                ...
                if token is None:
                    ...
                    elif code == 2 and asyn == 1:
                        return ResponseMock('blabla\r\nblabla\r\nblabla\r\nauthinfo1\r\nauthinfo2')
                elif token is not None and (code == 16 or code == 7) and asyn == 0:
                    return ResponseMock('00000')
        
        client = TplinkC80RouterTest('', '')
        client.authorize()
        
        encryption = client._encryption_state
    ...
    def test_get_firmware(self) -> None:
        ...
        class TplinkC80RouterTest(TplinkC80Router):
            def request(self, code: int, asyn: int, token: str = None, data: str = None) -> dict | None:
                ...
                if token is None:
                    ...
                    return ResponseMock('00000')
                # Response for firmware
                if code == 2 and asyn == 1 and token is not None:
                    return ResponseMock(encrypted_status_response)
        ...
        encryption = client._encryption_state
        encrypted_status_response = client._encrypt_body(encryption.key_aes, encryption.iv_aes, encryption.string_aes, 
                                                         response_status, encryption.seq, encryption.nn_rsa, encryption.ee_rsa).split('data=')[1]
    ...
    def test_get_status(self) -> None:
        ...
        encryption = client._encryption_state
        encrypted_status_response = client._encrypt_body(encryption.key_aes, encryption.iv_aes, encryption.string_aes, 
                                                         status_response_text, encryption.seq, encryption.nn_rsa, encryption.ee_rsa).split('data=')[1]
