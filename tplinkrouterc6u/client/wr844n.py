...
class TplinkWR844NRouter(TplinkC80Router):
    """Client for the TL-WR844N.

    The WR844N uses the same C80-style block API but answers data requests
    (firmware, status) with a **plain-text** block rather than an encrypted
    payload, and rejects encrypted data requests with error code 6 (see issue
    #59). Only the supports/authorize handshake is encrypted; data requests
    must be sent as plain text with the session token.
    """
    ...
    def get_firmware(self) -> Firmware:
        response_text = self._request_plaintext('0|1,0,0')
        device_datamap = dict(line.split(" ", 1) for line in response_text.split("\r\n")[1:-1])
        return Firmware(parse.unquote(device_datamap.get('hardVer', '')),
                        parse.unquote(device_datamap.get('modelName', '')),
                        parse.unquote(device_datamap.get('softVer', '')))
    ...
    def _request_plaintext(self, text: str) -> str:
        # The WR844N answers data requests with plain text and rejects an
        # encrypted body (error code 6). Send the raw request text with the
        # session token (asyn=1), matching the working web-UI call.
        response = self.request(2, 1, True, data=text)
